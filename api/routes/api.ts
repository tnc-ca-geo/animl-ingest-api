...
export default async function router(schema: any) {
  ...
  await schema.post(
    ...
    async (req: Request, res: Response) => {
      try {
        if (!process.env.LOGIN_ID || !process.env.LOGIN_TOKEN)
          throw new Err(500, null, "ID & Token haven't been configured on this API");
        if (
          (req.body.Id || req.body.id) !== process.env.LOGIN_ID ||
          (req.body.Token || req.body.token) !== process.env.LOGIN_TOKEN
        )
          throw new Err(401, null, "Unauthorized");

        return res.json({
          token: jwt.sign({ id: req.body.Id || req.body.id }, String(process.env.SECRET), {
            algorithm: "HS256",
            expiresIn: "7d",
          }),
  ...
  await schema.post(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        console.log(
          `Received image ${req.body.Metadata.File_name} from camera ${req.body.Camera_serial_number} taken at ${req.body.Metadata.date_time_taken}`,
        );
